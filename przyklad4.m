%% Przykład 3 : p(x) = -x^6 + 3x^4 - 2x^2 + 1
...
title(ax1,'Newton + Horner: p(x) stopnia 6','FontSize',14,'FontWeight','bold');
grid(ax1,'on'); box(ax1,'on');

plot(ax1, x0, ocenaHornera(wsp,x0), 'ro','MarkerFaceColor','r','MarkerSize',8);
text(ax1, x0+0.05, ocenaHornera(wsp,x0)+0.1, 'x_0','Color','r','FontSize',11);
plot(ax1, x_stat, ocenaHornera(wsp,x_stat), 'ks','MarkerFaceColor','k','MarkerSize',8);
text(ax1, x_stat+0.05, ocenaHornera(wsp,x_stat)+0.1, 'x^*','Color','k','FontSize',11);

t = linspace(x0-2, x0+2, 200);
